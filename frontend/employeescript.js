...
function employeeSaveToLocal() {
    localStorage.setItem("moderntech_employees_v1", JSON.stringify(employeeAll));
}
...
async function employeeUpdate(employeeId) {
    ...
    try {
        ...
        const stored = localStorage.getItem("moderntech_employees_v1");
        employeeAll = stored ? JSON.parse(stored) : [];
        const index = employeeAll.findIndex(e => e.employeeId === employeeId);
        if (index !== -1) {
            employeeAll[index] = { ...employeeAll[index], name, position, department, salary, contact, startDate, employmentHistory: history };
            employeeSaveToLocal();
        }
        employeeShowToast(`${name} updated successfully`, "success");
        setTimeout(() => { window.location.href = "employees.html"; }, 1000);
    } catch (error) {
        employeeShowToast("Error updating employee", "error");
    }
}
...
function populatePositionSelect(dept) {
    ...
    options.forEach((p) => { const o = document.createElement("option"); o.value = p; o.textContent = p; posSelect.appendChild(o); });
    if (employeeToEdit && employeeToEdit.position) { posSelect.value = employeeToEdit.position; }
}
