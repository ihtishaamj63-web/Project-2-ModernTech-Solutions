...
// ------- Everything below is the existing timeoff.js -------
// (Only the localStorage parts have been replaced)
...
function initTimeOff() {
    ...
    // ---- REPLACED: loadSharedData() is now removed ----
    // We now use API data. The data from API replaces localStorage.
    ...
    // ---- REPLACED: saveData() now calls the API ----
    async function saveData() {
        // This is called after approve/deny/submit
        // The API endpoints handle saving, so we just refresh the view
        renderAllRequests();
        updateTabCounts();
    }
    ...
    function syncAttendanceOnApproval(emp, startDate, endDate) {
        // This function updates attendance when leave is approved
        // It now works with the data structure from the API
    ...
    // ---- REPLACED: handleApprove now uses API ----
    window.handleApprove = async function (employeeId, date) {
        ...
        try {
            // Find the timeoff_id from the request (we need it for the API)
            const result = await approveTimeOffAPI(request.timeoff_id || 1);
            if (result.success) {
                request.status = "Approved";
                syncAttendanceOnApproval(emp, request.date, request.endDate || request.date);
                renderAllRequests();
    ...
    // ---- REPLACED: handleDeny now uses API ----
    window.handleDeny = async function (employeeId, date) {
        ...
        try {
            const result = await denyTimeOffAPI(request.timeoff_id || 1, "Request denied");
            if (result.success) {
    ...
    window.handleCancel = function (employeeId, date) {
        // Cancel removes the request from the list
        const emp = employees.find((e) => e.employeeId === employeeId);
        if (!emp) return;
    ...
    // ---- REPLACED: submitNewRequest now uses API ----
    async function submitNewRequest() {
        ...
        try {
            // Call the API instead of saving to localStorage
